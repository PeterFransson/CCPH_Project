function check_boundary(para::Array{Float64,1},ranges::Array{Tuple{Float64,Float64},1})
    return any(x->x==false,first.(ranges).<para.<last.(ranges)) 
end

# Sampler in R^d
function RoSampler(log_p::Function, n_samples::Integer, x0::Array{Float64,1},ranges::Array{Tuple{Float64,Float64},1};n_burn_in::Integer=1000)
    ...
    for k = 1:n_steps
        ...
        while check_boundary(r.y,ranges)
            AdaptiveMCMC.draw!(r, s)
        end    
...
function run_sampler(file_name::String,ranges::Array{Tuple{Float64, Float64},1},parasym::Array{Symbol,1},
    x0_F::Array{Float64,1},x0_C::Array{Float64,1};
    ...
    n_samples, x0_F,ranges;n_burn_in=n_burn_in)
    ...
    n_samples, x0_C,ranges;n_burn_in=n_burn_in)
...
function plot_samples(file_name::String,parasym::Array{Symbol,1})
    ...
    savefig("./plots/"*file_name_F*"_his.svg")

    histogram(transpose(samples_C),layout=size(samples_C,1))
    savefig("./plots/"*file_name_C*"_his.svg")
