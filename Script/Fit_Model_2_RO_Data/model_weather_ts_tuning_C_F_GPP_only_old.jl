...
function Run_RO_C_F_CCPH(par::Array{Float64,1},weatherts_F::WeatherTS,
    ...
    for j = 1:sim_steps+1
        ...
        B = data_F.B[data_ind]
        model,kinetic = Initi_model_struct(H,N,Wf,B,αf,β₁,β₂,Nₛ,rₘ,a_Jmax,b_Jmax,i,Kₛᵣ₀)
        GPP_model_F[j] = CalcGPP!(j,model,weatherts_F,kinetic)
    end  
    ...
    for j = 1:sim_steps+1
        ...
        GPP_model_C[j] = CalcGPP!(j,model,weatherts_C,kinetic)
    end  
...
function run_simple_trait_model_C_F_tuning_RO_ts()
    ...
    x::Array{Float64,1}->Post_distri_RO_C_F_CCPH(x::Array{Float64,1},RO_data),q_vec,"RO_C_F_GPP_Only_20211119";
