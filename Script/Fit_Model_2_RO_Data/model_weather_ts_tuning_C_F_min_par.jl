...
function Run_RO_C_F_CCPH(par::Array{Float64,1},weatherts_F::WeatherTS,
    ...
    αf,β₁,β₂,rₘ,b_Jmax,i,Kₛᵣ₀ = 460.0,1.27,-0.27,24.0,0.0,1.0,8.0
    Nₛ,a_Jmax,Nₛ_C = par    
...
function q_distri_RO_C_F_CCPH(par;sd::Float64=25.0)
    return par.+rand(Normal(),3).*[0.03,0.044,0.02]/sd          
end
...
function run_simple_trait_model_C_F_tuning_RO_ts()
    ...
    #=
    q_vec = [0.03,0.044,0.02]/25.0
    metropolis_mcmc(x_current,P_current,
    x::Array{Float64,1}->Post_distri_RO_C_F_CCPH(x::Array{Float64,1},RO_data),q_vec,"RO_C_F_20211108";
...
function run_C_F_ts_mean()
    ...
    RO_data = Load_RO_data()  
    samples = load("output/RO_C_F_20211108.jld","samples_container")    
    P_samples = load("output/RO_C_F_20211108.jld","P_samples_container")
    ...
    savefig("./plots/simple_CCPH_RO_C_F_ts_tuned_20211108.svg")
end
#=
