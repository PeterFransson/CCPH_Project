...
function Run_RO_C_F_CCPH(par::Array{Float64,1},weatherts_F::WeatherTS,
    ...
    for j = 1:sim_steps+1        
        ...
        B = data_F.B[data_ind]  
        =#      
        model,kinetic = Initi_model_struct(data_ind,data_F,αf,β₁,β₂,Nₛ,rₘ,a_Jmax,b_Jmax,i,Kₓₗ₀)
    ...
    for j = 1:sim_steps+1
        ...
        =#
        model,kinetic = Initi_model_struct(data_ind,data_C,αf,β₁,β₂,Nₛ_C,rₘ,a_Jmax,b_Jmax,i,Kₓₗ₀)
...
function RO_C_F_CCPH_ModelOutput(par::Array{Float64,1},weatherts_F::WeatherTS,
    ...
    αf,β₁,β₂,Nₛ,rₘ,a_Jmax,b_Jmax,i,Kₓₗ₀,Nₛ_C = par[1:10]
    ...
    for j = 1:sim_steps+1
        ...
        Wf = data_F.Wf[data_ind]
        B = data_F.B[data_ind]
        model,kinetic = Initi_model_struct(H,N,Wf,B,αf,β₁,β₂,Nₛ,rₘ,a_Jmax,b_Jmax,i,Kₓₗ₀)
...
function Check_para_RO_C_F_CCPH(par::Array{Float64,1})
    ...
    lo = [0.01,16.0,0.01,0.001,0.01,-8.0,3.0,10.0,3.0]

    up = [0.1,40.0,0.1,0.1,0.1,-1.0,15.0,20.0,15.0] 
...
function run_simple_trait_model_C_F_tuning_RO_ts()
    ...
    par_guess = [0.036975489745880365, 17.40739827323768, 0.04, 0.01,
    ...
    x::Array{Float64,1}->Post_distri_RO_C_F_CCPH(x::Array{Float64,1},RO_data,RO_ET_data),q_vec,"RO_C_F_GPP_Only_Few_Para_NoRoot_20220105";
