function Initi_model_struct(H,N,Wf,B,αf,β₁,β₂,Nₛ,rₘ,a_Jmax,b_Jmax,i,Kₛᵣ₀,Hc)
    cons = Constants()
...
function Initi_model_struct(j::Integer,data::RoData,
    αf::T,β₁::T,β₂::T,Nₛ::T,rₘ::T,a_Jmax::T,b_Jmax::T,i::T,Kₛᵣ₀::T,Hc::T) where {T<:Float64}
    data_ind = Find_data_ind(j)
    H = data.H[data_ind]
    N = data.N[data_ind]
    Wf = data.Wf[data_ind]
    B = data.B[data_ind]
    model,kinetic = Initi_model_struct(H,N,Wf,B,αf,β₁,β₂,Nₛ,rₘ,a_Jmax,b_Jmax,i,Kₛᵣ₀,Hc)    
...
function Run_RO_C_F_CCPH(par::Array{Float64,1},weatherts_F::WeatherTS,
    ...
    for j = 1:sim_steps+1
        ...
        B = data_F.B[data_ind]        
        model,kinetic = Initi_model_struct(H,N,Wf,B,αf,β₁,β₂,Nₛ,rₘ,a_Jmax,b_Jmax,i,Kₛᵣ₀,data_F.Hc)
        GPP_model_F[j], ET_model_F[j] = CalcGPP!(j,model,weatherts_F,kinetic)
    end  

    #Control stand
    GPP_model_C = zeros(sim_steps+1)
    ET_model_C = zeros(sim_steps+1)
    for j = 1:sim_steps+1
        data_ind = Find_data_ind(j)
        H = data_C.H[data_ind]
        N = data_C.N[data_ind]
        ...
        B = data_C.B[data_ind]
        model,kinetic = Initi_model_struct(H,N,Wf,B,αf,β₁,β₂,Nₛ_C,rₘ,a_Jmax,b_Jmax,i,Kₛᵣ₀,data_C.Hc)
        GPP_model_C[j], ET_model_C[j] = CalcGPP!(j,model,weatherts_C,kinetic)
    end  

    return GPP_model_F,GPP_model_C,ET_model_F,ET_model_C
end
...
function Calc_error_RO_C_F_CCPH(GPP_model_F::Array{Float64,1},data_F::RoData,
    GPP_model_C::Array{Float64,1},data_C::RoData,ET_model_F::Array{Float64,1},ET_model_C::Array{Float64,1})   
    ...
    ET_error_F =  1-calcR²(data_F.ET,ET_model_F)
    ...
    ET_error_C =  1-calcR²(data_C.ET,ET_model_C)
     
    return  GPP_error_F+GPP_error_C+ET_error_F+ET_error_C
end
...
function Post_distri_RO_C_F_CCPH(par::Array{Float64,1},RO_data::RO_raw_data,RO_ET_data::RO_Raw_ET_Data)
    if Check_para_RO_C_F_CCPH(par)
        ...
        try
            ...
            GPP_model_F,GPP_model_C,ET_model_F,ET_model_C = Run_RO_C_F_CCPH(par,weatherts_F,weatherts_C,data_F,data_C)              

            error_val = Calc_error_RO_C_F_CCPH(GPP_model_F,data_F,GPP_model_C,data_C,ET_model_F,ET_model_C)
...
function run_simple_trait_model_C_F_tuning_RO_ts()
    ...
    x::Array{Float64,1}->Post_distri_RO_C_F_CCPH(x::Array{Float64,1},RO_data,RO_ET_data),q_vec,"RO_C_F_GPP_Only_w_ET_20211201";
...
function run_C_F_ts_mean()
    ...
    RO_data = Load_RO_data()  
    RO_ET_data = RO_Raw_ET_Data()
    samples = load("output/RO_C_F_GPP_Only_w_ET_20211201.jld","samples_container")    
    P_samples = load("output/RO_C_F_GPP_Only_w_ET_20211201.jld","P_samples_container")
    ...
    plot(weatherts_F.date,data_F.ET)
    pl3 = plot!(weatherts_F.date,ET_model_F)
    plot(weatherts_C.date,data_C.ET)
    pl4 = plot!(weatherts_C.date,ET_model_C)
